...
func TestUpdateProduct(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/products", strings.NewReader(`{"name":"Old","price":1}`))
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)

	req = httptest.NewRequest("PUT", "/products/1", strings.NewReader(`{"name":"New","price":2}`))
	rr = httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestUpdateProductNotFound(t *testing.T) {
	...
	req := httptest.NewRequest("PUT", "/products/999", strings.NewReader(`{"name":"New","price":2}`))
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
